...
import { sendMissingBackupAlert } from './email';
import { db } from './db';
import type { NewSupabaseBackup } from '$lib/types';
...
export async function checkBackups(): Promise<{ backupsFound: number; todayOk: boolean }> {
	...
	const todayBackup = backups.find((b) => {
		const backupDate = b.inserted_at.split('T')[0];
		return backupDate === todayStr && b.status === 'COMPLETED';
	});

	// 4. Zo niet, stuur waarschuwing
	if (!todayBackup) {
		console.warn(`[cron] Geen succesvolle back-up gevonden voor ${todayStr}`);
		await sendMissingBackupAlert(todayStr);
	} else {
		console.log(`[cron] Back-up van vandaag (${todayStr}) OK`);
	}

	return { backupsFound: backups.length, todayOk: !!todayBackup };
}
